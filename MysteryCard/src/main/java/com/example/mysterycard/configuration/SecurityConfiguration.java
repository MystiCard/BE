...
public class SecurityConfiguration {
 ...
 "/swagger-ui/**", "/swagger-ui.html", "/api/files/upload","/api/payments/*","/api/auth/verify-email","/api/auth/send-verify-code","/api/auth/password-endcode/*"};

    @Bean
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.authorizeHttpRequests(auth -> {
            auth.requestMatchers(PUBLIC_API).permitAll()
                    .requestMatchers(USER_API).hasAnyRole("USER", "ADMIN","SHIPPER")
